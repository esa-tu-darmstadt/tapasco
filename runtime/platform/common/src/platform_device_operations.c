...
platform_res_t default_alloc_host(platform_devctx_t *devctx,
                                  size_t const len,
                                  platform_mem_addr_t *addr,
                                  platform_alloc_flags_t const flags)
{
                                ...
                                platform_ctl_flags_t const flags)
{
	default_platform_t *pp = (default_platform_t *)devctx->private_data;
	volatile void *r;
	DEVLOG(devctx->dev_id, LPLL_CTL, "addr = " PRIctl ", length = %zu", addr, length);

	if (IS_BETWEEN(addr, pp->regspace.arch.base, pp->regspace.arch.high)) {
		r = (void *) (((uintptr_t) pp->arch_map) + (addr - pp->regspace.arch.base));
	}
	else if (IS_BETWEEN(addr, pp->regspace.platform.base, pp->regspace.platform.high)) {
		r = (void *) (((uintptr_t) pp->plat_map) + (addr - pp->regspace.platform.base));
	}
	else if (IS_BETWEEN(addr, pp->regspace.status.base, pp->regspace.status.high)) {
		r = (void *) (((uintptr_t) pp->status_map) + (addr - pp->regspace.status.base));
	}
	else {
                                 ...
                                 platform_ctl_flags_t const flags)
{
	...
	if (IS_BETWEEN(addr, pp->regspace.arch.base, pp->regspace.arch.high))
		r = (volatile void *) (((uintptr_t) pp->arch_map) + (addr - pp->regspace.arch.base));
	else if (IS_BETWEEN(addr, pp->regspace.platform.base, pp->regspace.platform.high))
		r = (volatile void *) (((uintptr_t) pp->plat_map) + (addr - pp->regspace.platform.base));
	else {
		DEVERR(devctx->dev_id, "invalid platform address: 0x%08lx", (unsigned long)addr);
		return PERR_CTL_INVALID_ADDRESS;
	}
...
static
void default_unmap(default_platform_t *platform)
{
...
static
platform_res_t default_map(default_platform_t *platform)
{
	assert(platform->devctx);
	assert(platform->devctx->fd_ctrl);
	platform->arch_map = mmap(NULL,
	                          platform->regspace.arch.size,
	                          PROT_READ | PROT_WRITE | PROT_EXEC,
	                          MAP_SHARED,
	                          platform->devctx->fd_ctrl,
	                          4096);
	if (platform->arch_map == MAP_FAILED) {
	...
	DEVLOG(platform->devctx->dev_id, LPLL_DEVICE, "successfully mapped architecture");

	platform->plat_map = mmap(NULL,
	                          platform->regspace.platform.size,
	                          PROT_READ | PROT_WRITE | PROT_EXEC,
	                          MAP_SHARED,
	                          platform->devctx->fd_ctrl,
	                          8192);
	...
	platform->status_map = mmap(NULL,
	                            platform->regspace.status.size,
	                            PROT_READ | PROT_WRITE | PROT_EXEC,
	                            MAP_SHARED,
	                            platform->devctx->fd_ctrl,
	                            0);
...
#define INIT_DEFAULT_PLATFORM		(default_platform_t) { \
	.arch_map       		= MAP_FAILED, \
	.plat_map       		= MAP_FAILED, \
...
platform_res_t request_device_size(platform_devctx_t const *devctx) {
	DEVLOG(devctx->dev_id, LPLL_MM, "Reading size of design components from driver.");
	struct tlkm_size_cmd cmd = { .arch = 0, .status = 0, .platform = 0 };
	long ret = ioctl(devctx->fd_ctrl, TLKM_DEV_IOCTL_SIZE, &cmd);
	if (ret) {
		DEVERR(devctx->dev_id, "error reading design size: %s (%d)", strerror(errno), errno);
		return PERR_TLKM_ERROR;
	}
...
platform_res_t default_init(platform_devctx_t *devctx, size_t offboard_memory) {
	default_platform_t *pp = (default_platform_t *)malloc(sizeof(default_platform_t));
