...
static int get_tlkm_version(platform_ctx_t *ctx, char *v, int v_buffer_length) {
  struct tlkm_ioctl_version_cmd c = {
      .version = "",
  };
  if (ioctl(ctx->fd_tlkm, TLKM_IOCTL_VERSION, &c)) {
    ERR("getting version from device driver failed: %s (%d)", strerror(errno),
        errno);
    return errno;
  }
...
static int enum_devs(platform_ctx_t *ctx) {
  int r = 0;
  assert(ctx);
...
static platform_res_t get_dev(platform_ctx_t *ctx,
...
static platform_res_t init_platform(platform_ctx_t *ctx) {
  ...
  if (get_tlkm_version(ctx, ctx->version, TLKM_VERSION_SZ))
    goto err_ioctl;
...
static void deinit_platform(platform_ctx_t *ctx) {
  close(ctx->fd_tlkm);
...
platform_res_t _platform_init(const char *const version, platform_ctx_t **ctx) {
  ...
  *ctx = (platform_ctx_t *)calloc(sizeof(**ctx), 1);
  ...
  if ((r = init_platform(*ctx)) != PLATFORM_SUCCESS) {
    ERR("failed to initialize platform: %s (" PRIres ")", platform_strerror(r),
        r);
    goto err_init;
  }
...
platform_res_t platform_enum_devices(platform_ctx_t *ctx, size_t *num_devs,
...
platform_res_t platform_create_device(platform_ctx_t *ctx,
  ...
  if ((r = ioctl(ctx->fd_tlkm, TLKM_IOCTL_CREATE_DEVICE, &c))) {
    ERR("could not create device #" PRIdev ": %s (%d)", dev_id, strerror(errno),
        errno);
    return PERR_TLKM_ERROR;
  }
  LOG(LPLL_TLKM, "created device #" PRIdev ", initializing device context ...",
      dev_id);
  if ((res = platform_devctx_init(ctx, dev_id, mode, &ctx->devctx[dev_id])) !=
      PLATFORM_SUCCESS) {
...
void platform_destroy_device_by_id(platform_ctx_t *ctx,
  ...
  if ((r = ioctl(ctx->fd_tlkm, TLKM_IOCTL_DESTROY_DEVICE, &c))) {
    ERR("could not destroy device #" PRIdev ": %s (%d)", dev_id,
