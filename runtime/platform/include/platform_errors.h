...
#define PLATFORM_ERRORS                                                        \
  ...
  _X(PERR_OPEN_FILES_HARD_LIMIT, -4,                                           \
     "hard limit for number of open files insufficient")                       \
  _X(PERR_OPEN_FILES_SET_LIMIT, -5,                                            \
     "soft limit for number of open files could not be increased")             \
  ...
  _X(PERR_MEM_NO_SUCH_HANDLE, -10,                                             \
     "device memory with specified handle does not exist")                     \
  _X(PERR_MEM_NO_SUCH_BUFFER, -11,                                             \
     "device memory buffer with specified id does not exist")                  \
  ...
  _X(PERR_STATUS_CORE_NOT_FOUND, -23,                                          \
     "TaPaSCO status core not found, invalid bitstream")                       \
...
typedef enum { PLATFORM_ERRORS } platform_error_t;
#undef _X
