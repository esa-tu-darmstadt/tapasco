...
int main(int argc, const char *argv[]) {
  ...
  if (cli.key_exists("help")) {
    ...
    std::cout << "	--debug:             enables debugging output"
              << std::endl;
    std::cout << "	--convert_grayscale: convert RGB image to grayscale"
              << std::endl;
    std::cout << "	--operation=<val>:   image operation to perform"
              << std::endl;
    std::cout << "	--outfile:           name of the output image"
              << std::endl;
    std::cout << std::endl;
    ...
    std::cout
        << "	./imgproc --operation=GAUSS3x3 --convert_grayscale image.png"
        << std::endl;
    return EXIT_SUCCESS;
  }
  ...
  if (cli.key_exists("operation")) {
    ...
    std::cerr << "ERROR: choose operation to perform (--help for more info)"
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  //  - Index of the signal that has been allocated to access the signal in the
  //  mapped dma_mem
  ...
  // Fetch write pointer from the hardware -> Make sure we enqueue our packages
  // at the correct location
  uint64_t current_write_ptr = arbiter_space[26];
  if (debug_mode) {
    std::cout << "Fetched old write pointer from hardware as "
              << current_write_ptr << std::endl;
  }
  ...
  *doorbell = current_write_ptr;
  ...
  uint8_t *dma_area =
      static_cast<uint8_t *>(mem.getVirtualAddr()) + offset_image;
  ...
  if (debug_mode) {
    std::cout << "Using " << actual_size << "byte of " << max_size << " byte."
              << std::endl;
  }
  ...
  for (unsigned int j = 0; j < (unsigned int)src_img.height(); ++j) {
    for (unsigned int i = 0; i < (unsigned int)src_img.width(); ++i) {
      ...
      if (convert_grayscale) {
        ...
        uint16_t gs_val =
            static_cast<uint16_t>(luminance * ((1 << pixel_bitwidth) - 1));
  ...
  hsa_kernel_dispatch_packet_t *pktQueue =
      (hsa_kernel_dispatch_packet_t *)queue.get_package_queue();
  hsa_kernel_dispatch_packet_t *packet =
      pktQueue + calculate_packet_index(current_write_ptr);
  ...
  // Adding kernags
  uint64_t *kernargs =
      (uint64_t *)((uint64_t)mem.getVirtualAddr() + offset_kernargs);
  ...
  *(((uint8_t *)kernargs) + 17) = (uint8_t)CLAMP_TO_ZERO;
  ...
  uint32_t packet_format =
      ((uint32_t)setup(2) << 16) | header(HSA_PACKET_TYPE_KERNEL_DISPATCH);
  ...
  if (debug_mode) {
    ...
    std::cout << std::hex << "Kernel Object: " << packet->kernel_object
              << std::endl;
    std::cout << std::hex << "Size X: " << packet->grid_size_x << std::endl;
    std::cout << std::hex << "Size Y: " << packet->grid_size_y << std::endl;
    std::cout << std::hex << "Signal: " << packet->completion_signal.handle
              << std::endl;
    std::cout << std::hex << "Kernarg Address: " << packet->kernarg_address
              << std::endl;
    std::cout << std::hex << "Kernarg 0: " << std::hex << kernargs[0]
              << std::dec << std::endl;
    std::cout << std::hex << "Kernarg 1: " << std::hex << kernargs[1]
              << std::dec << std::endl;
    std::cout << std::hex << "Kernarg 2: " << std::hex << kernargs[2]
              << std::dec << std::endl;
  }
  ...
  for (unsigned int y = 0; y < (unsigned int)src_img.height(); ++y) {
    for (unsigned int x = 0; x < (unsigned int)src_img.width(); ++x) {
      if (convert_grayscale) {
        ...
        auto r = RGBPixel{(uint8_t)intensity, (uint8_t)intensity,
                          (uint8_t)intensity};
