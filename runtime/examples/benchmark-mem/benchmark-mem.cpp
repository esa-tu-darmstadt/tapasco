...
inline tapasco::tapasco_device_copy_flag_t
operator|(tapasco::tapasco_device_copy_flag_t a,
          tapasco::tapasco_device_copy_flag_t b) {
  return static_cast<tapasco::tapasco_device_copy_flag_t>(static_cast<int>(a) |
                                                          static_cast<int>(b));
}
...
static inline void tapasco_transfer(void *d) {
  ...
  if (Tapasco.alloc(h, chunk_sz, tapasco::TAPASCO_DEVICE_ALLOC_FLAGS_NONE) !=
  ...
  switch (mode - 3) {
  case 0: /* read-only */
    if (Tapasco.copy_from(h, d, chunk_sz,
                          tapasco::TAPASCO_DEVICE_COPY_BLOCKING) !=
        tapasco::TAPASCO_SUCCESS)
      errors++;
    break;
  case 1: /* write-only */
    if (Tapasco.copy_to(d, h, chunk_sz,
                        tapasco::TAPASCO_DEVICE_COPY_BLOCKING) !=
        tapasco::TAPASCO_SUCCESS)
      errors++;
    break;
  ...
  if (h)
    Tapasco.free(h, tapasco::TAPASCO_DEVICE_ALLOC_FLAGS_NONE);
}
...
static void print_header(void) {
  cout << "Allocation Size (KiB),virt. R (MiB/s),virt. W (MiB/s),virt. R+W "
          "(MiB/s),DMA R (MiB/s),DMA W (MiB/s),DMA R+W (MiB/s)"
       << endl;
}
...
static void print_line(ul const *times) {
  cout << chunk_sz / 1024 << ", "
       ...
       << (TRANSFER_SZ / (1024 * 1024)) / (times[5] / 1000000.0) << endl;
}
...
int main(int argc, char **argv) {
  ...
  for (auto pw = UPPER_BND; pw >= LOWER_BND; --pw) {
    ...
    for (mode = 0; mode <= 5; ++mode) {
      ...
      for (auto &f : fs)
        f.get();
      auto run_d = chrono::duration_cast<chrono::microseconds>(
