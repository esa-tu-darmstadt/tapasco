...
public:
  LocalMemoryScreen(Tapasco &tapasco)
      : MenuScreen("PE-local memory monitor", vector<string>()),
        tapasco(tapasco) {
    tapasco.info(&info);
    keypad(stdscr, TRUE);
    init_memories();
...
protected:
  ...
  virtual void render() {
    ...
    print_reversed(
        [&]() { mvprintw(row, (cols - title.length()) / 2, title.c_str()); });
    row += 2;
    for (auto &m : mem)
      render_mem(row++, *m);
    ++row;
    print_reversed(
        [&]() { mvprintw(row, (cols - bottom.length()) / 2, bottom.c_str()); });
  }
...
private:
  ...
  int loadsave(bool const load) {
    ...
    cbreak();
    attron(A_REVERSE);
    mvprintw(rows / 2 + 1, (cols - lconf.length()) / 2,
             load ? lconf.c_str() : sconf.c_str());
    attroff(A_REVERSE);
    ...
    do {
      ...
      if (chr == 'y')
        return load ? load_memory(*mem[curr_sel], tmp)
                    : save_memory(*mem[curr_sel], tmp);
    } while (c == ERR);
    cbreak();
    noecho();
    clear();
  ...
  int save_memory(memory_t const &m, string const &fn) {
    ...
    if (!data)
      throw Tapasco::tapasco_error(TAPASCO_ERR_OUT_OF_MEMORY);
    memset((void *)data, 0, m.size);
    tapasco_res_t r =
        tapasco_device_copy_from_local(tapasco.device(), 0, data, m.size,
                                       TAPASCO_DEVICE_COPY_FLAGS_NONE, m.slot);
  ...
  int load_memory(memory_t const &m, string const &fn) {
    ...
    if (inf) {
      ...
      mvprintw(rows / 2, (cols - copying.length()) / 2, copying.c_str());
      tapasco_res_t r =
          tapasco_device_copy_to_local(tapasco.device(), data, 0, m.size,
                                       TAPASCO_DEVICE_COPY_FLAGS_NONE, m.slot);
      delete data;
      clear();
      if (r != TAPASCO_SUCCESS)
