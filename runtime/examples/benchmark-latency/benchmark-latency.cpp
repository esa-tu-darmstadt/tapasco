...
static inline uint32_t clock_period(void) {
  ...
  if (period == 0.0) {
    ...
    if (!ifs.good()) {
      cerr << "WARNING: could not open /sys/class/fclk/fclk0/set_rate, using "
              "TAPASCO_FREQ"
           << endl;
...
static inline uint32_t platform_run(uint32_t cc) {
  ...
  if (platform_write_ctl(sb + 0x20, 4, &cc, PLATFORM_CTL_FLAGS_NONE) !=
      PLATFORM_SUCCESS)
    __atomic_fetch_add(&errors, 1, __ATOMIC_SEQ_CST);
  if (platform_write_ctl_and_wait(sb, 4, &start, 0, PLATFORM_CTL_FLAGS_NONE) !=
      PLATFORM_SUCCESS)
    __atomic_fetch_add(&errors, 1, __ATOMIC_SEQ_CST);
  // ack interrupt
  if (platform_write_ctl(sb + 0xc, 4, &start, PLATFORM_CTL_FLAGS_NONE) !=
      PLATFORM_SUCCESS)
    __atomic_fetch_add(&errors, 1, __ATOMIC_SEQ_CST);
  if (platform_read_ctl(sb + 0x10, 4, &start, PLATFORM_CTL_FLAGS_NONE) !=
      PLATFORM_SUCCESS)
    __atomic_fetch_add(&errors, 1, __ATOMIC_SEQ_CST);
  if (start != cc)
    cerr << "WARNING: found return value of " << start
         << " instead of expected " << cc << endl;
  return start;
}

static inline void print_header(void) {
  cout << "Wait time (ns), Stopwatch Latency TPC (ns), Stopwatch Latency "
          "Platform (ns), "
       << "IRQ Ack. Latency TPC (ns), IRQ Ack. Latency Platform (ns)" << endl;
}

// static inline void print_line(double clk, unsigned long long t1, unsigned
// long long t2)
template <typename T1, typename T2>
static inline void print_line(const uint32_t clk, const T1 &t1, const T1 &t2,
                              const T2 &t3, const T2 &t4) {
  cout << fixed << setprecision(1) << clk << ", " << t1 << ", " << t2 << ", "
       << t3 << ", " << t4 << endl;
}

static inline void print_usage(void) {
  cerr << "Usage: benchmark-latency [<MIN_TIME> [<MAX_TIME> [<TIME_STEPS> "
          "[<ITERATIONS>]]]] with"
       << endl
       << "\t<MIN_TIME>   = minimum kernel runtime in ns                "
          "(default: 10ns)"
       << endl
       << "\t<MAX_TIME>   = maximum kernel runtime in ns                "
          "(default: 10000ns)"
       << endl
       << "\t<TIME_STEPS> = number of equidistant sampling points       "
          "(default:10)"
       << endl
       << "\t<ITERATIONS> = number of iterations at each sampling point "
          "(default:1000)"
       << endl
       << endl;
}
...
static inline void print_args(struct config_t const *cfg) {
  cerr << "Configuration:" << endl
       ...
       << "\titerations          = " << cfg->iterations << endl
       << endl;
}
...
static inline void parse_args(int argc, char **argv, struct config_t *cfg) {
  ...
  if (argc > 1)
    cfg->min = stoi(string(argv[1]));
  if (argc > 2)
    cfg->max = stoi(string(argv[2]));
  if (argc > 3)
    cfg->time_steps = stoi(string(argv[3]));
  if (argc > 4)
    cfg->iterations = stoi(string(argv[4]));
  print_args(cfg);
}
...
int main(int argc, char **argv) {
  ...
  for (unsigned int i = 0; i < cfg.time_steps; ++i, clk += clk_step) {
    ...
    auto run_d = chrono::duration_cast<chrono::nanoseconds>(
        chrono::high_resolution_clock::now() - run_start);
    auto run_time =
        run_d.count() / (double)cfg.iterations - (double)rounded_clk;
    ...
    auto papi_d = chrono::duration_cast<chrono::nanoseconds>(
        chrono::high_resolution_clock::now() - papi_start);
    auto papi_time =
        papi_d.count() / (double)cfg.iterations - (double)rounded_clk;

    // cerr << "platform_latencies = " << platform_latencies << endl;

    if (ns_to_cd(rounded_clk) != rounded_cd)
      cerr << " FUUUUUUUUUUUUUUUUUUUUUUUUUUU" << endl;
    run_latencies *= clock_period();
    platform_latencies *= clock_period();
    run_latencies /= cfg.iterations;
    platform_latencies /= cfg.iterations;
    print_line(rounded_clk, run_time, papi_time, run_latencies,
               platform_latencies);
  }
  auto total_d = chrono::duration_cast<chrono::microseconds>(
      chrono::high_resolution_clock::now() - start);
  cerr << "Total duration: " << total_d.count() << " us, errors: " << errors
       << endl;
}
