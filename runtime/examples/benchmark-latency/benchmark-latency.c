...
static inline double clock_period(void) {
  ...
  if (period == 0.0) {
    ...
    if (fd == -1) {
      fprintf(stderr, "WARNING: could not open /sys/class/fclk/fclk0/set_rate, "
                      "using TAPASCO_FREQ\n");
...
static inline void tapasco_run(uint32_t cc) {
  ...
  if (tapasco_device_job_launch(
          dev, j_id, TAPASCO_DEVICE_JOB_LAUNCH_BLOCKING) != TAPASCO_SUCCESS)
...
static inline void print_header(void) {
  printf("Kernel time (ns), Kernel time (cycles), Average Latency TPC (us), "
         "Average Latency Platform (us)\n");
}

static inline void print_line(double clk, unsigned long long t1,
                              unsigned long long t2) {
  printf("%3.2f, %lu, %llu, %llu\n", clk, ns_to_cd(clk), t1, t2);
}

static inline void print_usage(void) {
  fprintf(stderr, "Usage: benchmark-latency [<MIN_TIME> [<MAX_TIME> "
                  "[<TIME_STEPS> [<ITERATIONS>]]]] with\n"
                  ...
                  " (default:10)\n"
                  "\t<ITERATIONS> = number of iterations at each sampling "
                  "point (default:1000)\n\n");
}
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < cfg.time_steps; ++i, clk += clk_step) {
    ...
    TIMER_STOP(run)
    times[i] =
        (TIMER_USECS(run) - (clk * cfg.iterations / 1000)) / cfg.iterations;

    TIMER_START(papi_run)
    for (int j = 0; j < cfg.iterations; ++j)
      platform_run(ns_to_cd(clk));
    TIMER_STOP(papi_run)
    unsigned long long int papi_time =
        (TIMER_USECS(papi_run) - (clk * cfg.iterations / 1000)) /
        cfg.iterations;
