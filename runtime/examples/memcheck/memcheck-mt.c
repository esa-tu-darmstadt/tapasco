...
static size_t const arr_szs[] = {1,    2,    8,    10,   16,
...
static void *test_thread(void *p) {
  ...
  while ((s = __atomic_sub_fetch(&runs, 1, __ATOMIC_SEQ_CST)) > 0) {
    ...
    //		sizeof(rarr));
    tapasco_res_t res = tapasco_device_copy_to(
        dev, arr, h, arr_szs[s] * sizeof(int), TAPASCO_DEVICE_COPY_BLOCKING);
    if (res == TAPASCO_SUCCESS) {
      // printf("%ld: copy to successful, copying from ...\n", s);
      res = tapasco_device_copy_from(dev, h, rarr, arr_szs[s] * sizeof(int),
    ...
    if (!merr)
      ...
      printf(/*stderr,*/
             "%ld: FAILURE: array size %zd (%zd byte) not ok.\n", s, arr_szs[s],
...
int main(int argc, char **argv) {
  if (argc < 2) {
    fprintf(stderr, "Usage: memcheck-mt <number of threads> [<number of "
