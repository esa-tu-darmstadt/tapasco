...
from grpc_gen import sim_calls_pb2_grpc as sc_grpc, sim_calls_pb2 as sc, status_core_pb2 as status_core
...
class SimServer(sc_grpc.SimRequestServicer):
    """Implementation of the SimRequestServicer. The superclass is generated by gRPC. It should only
    be used in conjunction with grpc_server.
    Initializes the SimServer class and schedules a readout of the tapasco status memory.
    Since we know, that the content of this memory doesn't change between host software executions
    it can be retrieved once and consequently be served from "python-memory" without having to involve
    cocotb.

    Parameters
    ----------
    dut: cocotb.handle.HierarchyObject
        Handle to the TaPaSCo design provided by cocotb. Needed for the initialization of the interrupts.
    ...
    """
    def __init__(self, dut, axim, memory):
    ...
    def write_memory(self, request, context):
        """Services a write request to the memory connected to the TaPaSCo design

        Parameters
        ----------
        request: grpc_gen.read_write_pb2.WriteMemory
        context: ignored

        Returns
        -------
        grpc_gen.sim_calls_pb2.SimResponse:
            Response type Okay and response_payload grpc_gen.sim_calls_pb2.Void
        """
        resp = sc.SimResponse()
        event = Event()
    ...
    def read_memory(self, request, context):
        """Services a read request to the memory connected to the TaPaSCo design

        Parameters
        ----------
        request: grpc_gen.read_write_pb2.ReadMemory
        context: ignored

        Returns
        -------
        grpc_gen.sim_calls_pb2.SimResponse:
            Response type Okay and response_payload grpc_gen.read_write_pb2.ReadMemoryResponse with the
    ...
    def read_platform(self, request, context):
        """Services a read request to the platform memory of the TaPaSCo design. This memory
        is accessed via cocotb and needs to cross the boundary between python and the simulator.

        Parameters
        ----------
        request: grpc_gen.read_write_pb2.ReadPlatform
        context: ignored

        Returns
        -------
        grpc_gen.sim_calls_pb2.SimResponse:
            Response type Okay and response_payload grpc_gen.read_write_pb2.ReadPlatformResponse with the
            data corresponding to the read request
        """
        resp = sc.SimResponse(type=sc.SimResponseType.Okay)
        event = Event()
    ...
    def write_platform(self, request, context):
        """Services a write request to the platform memory of the TaPaSCo design. This memory
        is accessed via cocotb and needs to cross the boundary between python and the simulator.
    ...
    def register_interrupt(self, request, context):
        """Enalbes a specific interrupt to be tracked. The interrupt is identified by the number
        specified in the tapasco-status. The SimInterrupt instance corresponding to
        this interrupt will now count signals asserted by it.

        Parameters
        ----------
        request: grpc_gen.sim_calls_pb2.RegisterInterrupt
        context: ignored

        Returns
        -------
        grpc_gen.sim_calls_pb2.SimResponse:
            Response type Okay and response_payload grpc_gen.sim_calls_pb2.RegisterInterrupt
        """
    ...
    def deregister_interrupt(self, request, context):
        """Deregisters a specific interrupt from being tracked.

        Parameters
        ----------
        request: grpc_gen.sim_calls_pb2.DeregisterInterrup
        context: ignored

        Returns
        -------
        grpc_gen.sim_calls_pb2.SimResponse:
            Response type Okay and response_payload grpc_gen.sim_calls_pb2.DeregisterInterrupt
        """
    ...
    def get_status(self, request, context):
        """Reads the status core inside the TaPaSCo design.

        Parameters
        ----------
        request: grpc_gen.sim_calls_pb2.Void
        context: ignored

        Returns
        -------
        grpc_gen.sim_calls_pb2.SimResponse:
            Response type Okay and response_payload grpc_gen.status_core_pb2.Status
        """
    ...
    def get_interrupt_status(self, request, context):
        """Returns the number of interrupts that have occured on a particular interrupt
        since the last read to that interrupt and resets the interrupt count on it.

        Parameters
        ----------
        request: grpc_gen.sim_calls_pb2.InterruptStatusRequest
        context: ignored

        Returns
        -------
        grpc_gen.sim_calls_pb2.SimResponse:
            Response type Okay and response_payload grpc_gen.sim_calls_pb2.InterruptStatus
        """
        resp = sc.SimResponse()
