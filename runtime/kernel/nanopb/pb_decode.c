...
static bool checkreturn pb_dec_fixed(pb_istream_t *stream,
				     const pb_field_iter_t *field);
					     ...
					     pb_field_iter_t *field)
{
	...
	case PB_HTYPE_ONEOF:
		...
		if (PB_LTYPE(field->type) == PB_LTYPE_STRING ||
			...
			if (!allocate_field(stream, field->pField,
					    field->data_size, 1))
				return false;
					      ...
					      pb_field_iter_t *field)
{
	...
	if (wire_type == PB_WT_STRING) {
		...
		return field->descriptor->field_callback(&substream, NULL,
							 field);
	}
}
					 ...
					 pb_field_iter_t *iter)
{
	...
	while (extension != NULL && pos == stream->bytes_left) {
		...
		if (extension->type->decode)
			status = extension->type->decode(stream, extension, tag,
							 wire_type);
		else
			status = default_extension_decoder(stream, extension,
							   tag, wire_type);
	...
	if (PB_LTYPE(type) == PB_LTYPE_EXTENSION) {
		...
		if (init_data) {
			if (PB_LTYPE(field->type) == PB_LTYPE_SUBMESSAGE) {
				...
				pb_field_iter_t submsg_iter;
				if (pb_field_iter_begin(&submsg_iter,
							field->submsg_desc,
							field->pData)) {
					if (!pb_message_set_to_defaults(
						    &submsg_iter))
						return false;
				}
			} else {
...
static bool pb_message_set_to_defaults(pb_field_iter_t *iter)
{
	...
	if (iter->descriptor->default_value) {
		defstream = pb_istream_from_buffer(
			iter->descriptor->default_value, (size_t)-1);
	...
	while (stream->bytes_left) {
		...
		if (!pb_field_iter_find(&iter, tag) ||
			...
			if (tag >= extension_range_start) {
				...
				if (tag >= extension_range_start) {
					...
					if (!decode_extension(stream, tag,
							      wire_type, &iter))
						return false;
		...
		if (PB_HTYPE(iter.type) == PB_HTYPE_REQUIRED &&
			...
			uint32_t tmp = ((uint32_t)1
					<< (iter.required_field_index & 31));
			fields_seen.bitfield[iter.required_field_index >> 5] |=
				tmp;
		}
			      ...
			      void *dest_struct, unsigned int flags)
{
	...
	if ((flags & PB_DECODE_DELIMITED) == 0) {
		...
		status =
			pb_decode_inner(&substream, fields, dest_struct, flags);
...
static bool pb_release_union_field(pb_istream_t *stream, pb_field_iter_t *field)
{
	...
	pb_size_t old_tag =
		*(pb_size_t *)field->pSize; /* Previous which_ value */
				      ...
				      const pb_field_iter_t *field)
{
	if (PB_LTYPE(field->type) == PB_LTYPE_UVARINT) {
		...
		if (field->data_size == sizeof(pb_uint64_t))
			...
			clamped = *(uint_least16_t *)field->pData =
				(uint_least16_t)value;
		else if (field->data_size == sizeof(uint_least8_t))
			clamped = *(uint_least8_t *)field->pData =
				(uint_least8_t)value;
		else
		...
		if (field->data_size == sizeof(pb_int64_t))
			...
			clamped = *(int_least16_t *)field->pData =
				(int_least16_t)svalue;
		else if (field->data_size == sizeof(int_least8_t))
			clamped = *(int_least8_t *)field->pData =
				(int_least8_t)svalue;
		else
			PB_RETURN_ERROR(stream, "invalid data_size");
					  ...
					  const pb_field_iter_t *field)
{
	...
	if (PB_HTYPE(field->type) == PB_HTYPE_REPEATED ||
		...
		status =
			pb_decode(&substream, field->submsg_desc, field->pData);
	else
		status = pb_decode_noinit(&substream, field->submsg_desc,
					  field->pData);
