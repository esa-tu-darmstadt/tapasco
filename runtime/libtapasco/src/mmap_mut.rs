...
pub unsafe fn tapasco_read_volatile(memory: &MemoryType, offset: isize) -> u64 {
    match memory {
        MemoryType::Sim(client) => client.read_platform(ReadPlatform{addr: offset as u64, num_bytes: 4}).unwrap() as u64,
        MemoryType::Mmap(mmap) => {
            let ptr = mmap.as_ptr().offset(offset);
            ptr.read_volatile().into()
        }
    }
}
