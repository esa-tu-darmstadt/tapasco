...
use simcalls::{RegisterInterrupt, sim_request_client::SimRequestClient};
...
pub mod sim {
    include!(concat!(env!("OUT_DIR"), "/tapasco.sim.rs"));
}
...
impl Device {
    ...
    pub fn new(
        ...
        println!("Open driver device file.");
        ...
        if !svm_in_use {
            println!("Initialize PE local memories.");
            for pe in &s.pe {
        ...
        println!("Initialize PE scheduler.");
        ...
        println!("Device creation completed.");
        let mut device = Self {
            id,
            vendor,
            product,
    ...
    pub fn acquire_pe(&self, id: PEId) -> Result<Job> {
        ...
        println!("Trying to acquire PE of type {}.", id);
        let pe = self.scheduler.acquire_pe(id).context(SchedulerSnafu)?;
        println!("Successfully acquired PE of type {}.", id);
    ...
    pub fn change_access(&mut self, access: tlkm_access) -> Result<()> {
        ...
        println!("Device {}: Trying to change mode to {:?}", self.id, access,);
        ...
        if access == tlkm_access::TlkmAccessExclusive {
            println!("Access changed to exclusive, resetting all interrupts.");
            self.scheduler.reset_interrupts().context(SchedulerSnafu)?;
        }

        println!("Successfully acquired access.");
        Ok(())
    }
    ...
    fn destroy(&mut self) -> Result<()> {
        if self.access != tlkm_access::TlkmAccessTypes {
            println!("Device {}: Removing access mode {:?}", self.id, self.access,);
            let mut request = tlkm_ioctl_device_cmd {
                dev_id: self.id,
                access: self.access,
            };
    ...
    pub fn get_available_platform_components(&self) -> Vec<String> {
        ...
        for p in &self.status.platform {
            println!("Found platform component {}.", p.name);
