...
public:
  AddressMapScreen(Tapasco &tapasco) : MenuScreen("Address Map", vector<string>()), tapasco(tapasco) {
...
protected:
  virtual void render() {
    ...
    row += 2;
    for (int c = PLATFORM_COMPONENT_STATUS; c <= PLATFORM_COMPONENT_DMA3; ++c, ++row) {
...
private:
  void render_platform_component(const int r, const char *name, platform_ctl_addr_t const b)
  {
    char tmp[256] = "";
  ...
  void render_slot(const int r, platform_slot_id_t const s, platform_ctl_addr_t const b)
  {
    char tmp[256] = "";
