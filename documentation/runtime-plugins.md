...
All runtime plugins must be written in Rust and implement the `Plugin` trait defined in [pugin.rs](../runtime/libtapasco/src/plugins/plugin.rs).
...
implement a FFI interface exposing all functionality in external C functions, as this is also done for the 
TaPaSCo runtime in [ffi.rs](../runtime/libtapasco/src/ffi.rs). Second, you have to write a C++ wrapper class holding
a pointer a pointer to the underlying Rust object and providing the plugin functionality with methods. This wrapper class must provide a static function `get_instance(Device *d)`
