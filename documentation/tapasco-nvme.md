...
or writes data back to these buffers as stated in the command. In this extension, we offer three
different options which memory is used for these DMA buffers: internal URAM, FPGA on-board DRAM, or host DRAM.
The following table may serve as a quick reference:
...
On-board DRAM | Limits achievable bandwidth, no URAM resources required, but still uses direct PCIe P2P transfers
...
for commands and one for responses in read and write direction, respectively. We recommend using
*512 bit* width for the TDATA signals of the read command, read response, and write command interfaces,
and *8 bit* for the write response interface. Other widths may work, but are not tested.

For read transactions from the NVMe device, the user PE issues a data packet with the following
...
[511:128] reserved
```
TLAST signal should be set. The NVMe Streamer IP will then return the read data as a continuous stream
on the read response channel.

For write transactions to the NVMe device, the first stream beat contains only the write address in bytes
on the NVMe device. Then all write data is sent over the stream until the end of the transaction is marked
by setting the TLAST signal by the user PE. We ignore the TKEEP signal, thus, all data beats must be properly packed.
After the write to the NVMe is fully executed, the NVMe Streamer IP sends one data beat on the write response
...
First, you need to specify the AXI4 Stream interfaces of your PE which are used for the read and write commands and responses.
...
The NVMe extension requires a few initialization steps during runtime. Hence, we provide a runtime plugin whose functionality is explained in the following.
...
In addition to initializing the FPGA part, you also have to set up the NVMe device, e.g., using a custom driver. **This is
not handled by TaPaSCo!** When setting up the I/O queue in the NVMe controller, you require the PCIe addresses of the command
...
calls. The first element of the returned tuples represents the submission queue PCIe address, and the second element represents the
