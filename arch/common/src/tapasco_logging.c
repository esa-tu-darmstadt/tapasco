...
#ifndef NDEBUG
static unsigned long int libtapasco_logging_level = ULONG_MAX;
static FILE *libtapasco_logging_file = NULL;
...
{
	if (!level || (level & libtapasco_logging_level)) {
...
static inline void handle_msg(struct log_msg_t *m)
{
	fprintf(libtapasco_logging_file ? libtapasco_logging_file : stderr, 
		m->lvl ? (m->lvl ^ 1 ?
			"%lld\t%d\t[libtapasco - info]\t%s" :
			"%lld\t%d\t[libtapasco - warning]\t%s") :
			"%lld\t%d\t [libtapasco - error]\t%s",
...
int tapasco_logging_init(void)
{
	...
	if (! is_initialized) {
		...
		libtapasco_logging_file = lgf ? fopen(lgf, "w+") : stderr;
		if (! libtapasco_logging_file) {
			libtapasco_logging_file = stderr;
