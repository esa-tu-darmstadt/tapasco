...
class EntityCacheSpec extends FlatSpec with Matchers {
  ...
  private def setupStructure(p: Path): (Path, Path, Path) = {
    ...
    Files.copy(TPC_HOME.resolve("report-examples").resolve("correct-synth1.rpt"), p.resolve("synth.rpt"))
    Files.copy(TPC_HOME.resolve("report-examples").resolve("correct-synth1.rpt"), p1.resolve("synth.rpt"))
    Files.copy(TPC_HOME.resolve("report-examples").resolve("correct-synth2.rpt"), p2.resolve("synth.rpt"))
    Files.copy(TPC_HOME.resolve("report-examples").resolve("correct-synth3.rpt"), p3.resolve("synth.rpt"))
    markForDeletion(p)
    (p1, p2, p3)
  }

  "All matching files in existing directory structure" should "be found and built correctly" in {
    val p = Files.createTempDirectory("tpc-entitycachespec-")
  ...
  "Cached entities" should "be rebuild upon modifications to source file" in {
    ...
    assert(r1.get.area map (_.resources.LUT == 538) getOrElse false)
    assert(r1.get.area map (_.available.SLICE == 13300) getOrElse false)
    Files.copy(TPC_HOME.resolve("report-examples").resolve("correct-synth3.rpt"), p.resolve("synth.rpt"),
    ...
    Files.copy(TPC_HOME.resolve("report-examples").resolve("invalid-synth1.rpt"), p3.resolve("synth.rpt"),
        StandardCopyOption.REPLACE_EXISTING)
    Thread.sleep(FS_SLEEP)
