 ...
 **/
package de.tu_darmstadt.cs.esa.threadpoolcomposer.itpc.common
...
class ReportManagerSpec extends FlatSpec with Matchers {
  private final val TPC_HOME = Paths.get(sys.env("TPC_HOME")).toAbsolutePath.normalize
  ...
  private def setupStructure(p: Path): (Path, Path, Path, Path) = {
    ...
    Files.copy(TPC_HOME.resolve("report-examples").resolve("correct-cosim1.rpt"),
      cosimPath.resolve("arraysum_cosim.rpt"))
    Files.copy(TPC_HOME.resolve("report-examples").resolve("correct-power1.rpt"),
      powerPath.resolve("power.rpt"))
    Files.copy(TPC_HOME.resolve("report-examples").resolve("correct-synth1.rpt"),
      synthPath.resolve("aes_export.xml"))
    Files.copy(TPC_HOME.resolve("report-examples").resolve("correct-timing.rpt"),
  ...
  "All reports in existing directory structure" should "be found and built correctly" in {
    import Entities._
    val p = Files.createTempDirectory("tpc-reportcache-").resolve("core")
    setupStructure(p)
  ...
  "Modification and deletion of source files" should "be reflected automatically" in {
    ...
    logger.info("{} -> {}", TPC_HOME.resolve("report-examples").resolve("correct-cosim2.rpt"): Any, r1.get.file)
    Files.copy(TPC_HOME.resolve("report-examples").resolve("correct-cosim2.rpt"), r1.get.file,
  ...
  "Changing the base path" should "be correctly reflected in the cache" in {
    import Entities._
    val p1 = Files.createTempDirectory("tpc-reportcache-")
    val p2 = Files.createTempDirectory("tpc-reportcache-")
    setupStructure(p1)
    val bpm = new BasePathManager
