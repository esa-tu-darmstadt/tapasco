...
class FileAssetManagerSpec extends FlatSpec with Matchers {
  ...
  "Changes in the base paths" should "be reflected in the caches" in {
    ...
    FileAssetManager.basepath(Entities.Architectures).set(
      TPC_HOME.resolve("json-examples").resolve("configTest").resolve("arch"))
    ...
    FileAssetManager.basepath(Entities.Platforms).set(
      TPC_HOME.resolve("json-examples").resolve("configTest").resolve("platform"))
    ...
    FileAssetManager.basepath(Entities.Kernels).set(
      TPC_HOME.resolve("json-examples").resolve("configTest").resolve("kernel"))
  ...
  "Creating new core.descriptions during runtime" should "be reflected in the caches" in {
    val p = Files.createTempDirectory("tpc-fileassetmanager-")
