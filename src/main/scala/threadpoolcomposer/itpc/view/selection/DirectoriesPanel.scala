 ...
 */
protected[itpc] class DirectoriesPanel extends BoxPanel(Orientation.Vertical) {
 ...
 **/
protected[itpc] object DirectoriesPanel {
   ...
   * Will set values in
   * [[de.tu_darmstadt.cs.esa.threadpoolcomposer.filemgmt.FileAssetManager]]
  ...
  sealed class EntityPathEditor(entity: Entity, select: Entity => Unit)
    ...
    private[this] val _fc    = new FileChooser(FileAssetManager.TPC_HOME.toFile) {
