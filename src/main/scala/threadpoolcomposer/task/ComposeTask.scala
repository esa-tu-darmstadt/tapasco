package de.tu_darmstadt.cs.esa.threadpoolcomposer.task
...
import  de.tu_darmstadt.cs.esa.threadpoolcomposer.Logging._
import  de.tu_darmstadt.cs.esa.threadpoolcomposer.dse.Heuristics
import  de.tu_darmstadt.cs.esa.threadpoolcomposer.itpc.common.LogFileTracker
...
class ComposeTask(
  ...
  private[this] implicit val _logger = de.tu_darmstadt.cs.esa.threadpoolcomposer.Logging.logger(getClass)
  ...
  private[this] val _logFile = logFile getOrElse "%s/%s.log".format(_outDir, "tpc")
  ...
  private def nodeExecution: Boolean = {
    ...
    if (debugMode.isEmpty) {
      _composerResult = Some(try   { composer.compose(composition, target, designFrequency) }
  ...
  private def slurmExecution: Boolean = {
    ...
    val job = Slurm.Job(
      ...
      commands = Seq("tpc --configFile %s".format(cfgFile.toString))
    )
