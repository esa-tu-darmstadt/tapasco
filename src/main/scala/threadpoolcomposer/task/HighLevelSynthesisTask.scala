...
import  de.tu_darmstadt.cs.esa.threadpoolcomposer.activity.hls._
import  de.tu_darmstadt.cs.esa.threadpoolcomposer.activity.hls.HighLevelSynthesizer.Implementation
...
import  de.tu_darmstadt.cs.esa.threadpoolcomposer.jobs.HighLevelSynthesisJob
...
class HighLevelSynthesisTask(val k: Kernel, val t: Target, val cfg: Configuration, hls: Implementation,
  ...
  private[this] implicit val logger = de.tu_darmstadt.cs.esa.threadpoolcomposer.Logging.logger(getClass)
  ...
  def job: Boolean = if (! slurm) {
    ...
    val job = Slurm.Job(
      ...
      commands = Seq("tpc --configFile %s".format(cfgFile.toString, k.name.toString))
    )
