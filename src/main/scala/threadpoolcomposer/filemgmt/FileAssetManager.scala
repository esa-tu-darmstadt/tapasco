...
final object FileAssetManager extends Publisher {
  private val _logger = de.tu_darmstadt.cs.esa.threadpoolcomposer.Logging.logger(getClass)
  ...
  lazy val TPC_HOME = try   { Paths.get(sys.env("TPC_HOME")).toAbsolutePath().normalize }
                      catch { case e: NoSuchElementException =>
                              _logger.error("FATAL: TPC_HOME environment variable is not set")
                              throw e }
  /* @} */
