package de.tu_darmstadt.cs.esa.threadpoolcomposer.slurm
import  de.tu_darmstadt.cs.esa.threadpoolcomposer.filemgmt._
import  de.tu_darmstadt.cs.esa.threadpoolcomposer.task.ResourceConsumer
import  de.tu_darmstadt.cs.esa.threadpoolcomposer.Logging._
import  de.tu_darmstadt.cs.esa.threadpoolcomposer.util.{Publisher, Template}
...
final object Slurm extends Publisher {
  ...
  final val slurmTemplate = FileAssetManager.TPC_HOME.resolve("common").resolve("slurm.job.template")
  /** Default output directory for SLURM-related outputs. */
  final val slurmOutput = FileAssetManager.TPC_HOME.resolve("slurm")
  ...
  def writeJobScript(job: Job, file: Path): Boolean = (catchDefault[Boolean](false, Seq(classOf[java.io.IOException]),
    ...
    jobScript("TPC_HOME") = FileAssetManager.TPC_HOME.toString
