...
final object FileAssetManager extends Publisher {
  ...
  def targetForReport(f: Path): Target = {
    ...
    Target(FileAssetManager.entities.architectures filter {
      _.name.equals(arch.toString)
    } head,
      FileAssetManager.entities.platforms filter {
        _.name.equals(platform.toString)
      } head)
  }
  ...
  val entities = new EntityManager(bpm) {
    this += _entityListener
  }
  /* @} */
  ...
  val reports = new ReportManager(basepath(Entities.Cores).get) {
    this += _reportListener
  }
  /* @} */
