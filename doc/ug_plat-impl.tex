...
An implementation of a \gloss{Platform} for \tapasco{} consists of several parts:
...
If necessary, the library can construct different designs, depending on the \code{TPC\_MODE}, e.g., to replace non-simulation cores with simulation models (see \code{tapasco\_get\_generate\_mode} Tcl call in \tblref{tbl:common.tcl}).
...
\tapasco{} offers a generic \gloss{Platform API} implementation in \code{platform/\allowbreak common} which connects a process to a remote, simulator-driven \gloss{Platform} using a client-server library based on socket inter-process communication (see \secref{sec:lls-sim} for details).
...
However, \tapasco{} does not impose any requirements regarding the realization; required is only that the shared library implementing the \gloss{Platform API} can be run in user mode.
