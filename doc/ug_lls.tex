...
\tapasco{} calls \code{platform\_generate}, which in turn configures and starts the bitstream generation using Vivado.
...
To decouple the application code and the simulator, \tapasco{} provides a client-server implementation of the \gloss{Platform API}, which connects a client-side library via \emph{socket inter-process communication (IPC)} to the server-side DPI library.
