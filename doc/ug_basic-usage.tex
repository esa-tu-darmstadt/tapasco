...
\tapasco{} can be run in \tapascohome{} via \gloss{sbt} and provides two general commands:
...
By default, \tapasco{} executes in \emph{batch mode}, i.e., it scans the currently set base directories for platform, architecture, kernel and bitstream descriptions, which must be named \code{platform|\allowbreak architecture|\allowbreak kernel|\allowbreak  bitstream.description}, but may reside in arbitrarily nested subdirectories of their corresponding base directory (cf. \figref{fig:dirs}).
%
\begin{figure}
  \centering%
...
\end{lstlisting}
%
\tapasco{} will scan the default directory \code{kernel/} for a \code{kernel.description} defining \code{add} and will report an error if it could not be found, or multiple conflicting definitions were found.
...
\end{lstlisting}
%
If the kernel was found, \tapasco{} will continue in the same manner to look for an \gloss{Architecture Description} called \code{baseline}.
...
E.g., the previously used commands can be moved into a file \code{myconfig.d} in \tapascohome{} with the following contents:
%
