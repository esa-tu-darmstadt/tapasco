...
\tapasco{} aims to integrate into this development flow after the kernel extraction phase, as \figref{fig:tapasco-overview} illustrates.
%
\begin{figure}%[h!]
  \centering\includegraphics[]{tikz/tapasco-overview}
...
\medskip
The upper half of \figref{fig:tapasco-overview} depicts the coarse grained steps for deriving a suitable FPGA configuration from a user application, while the bottom half denotes the intermediate products at each step.
...
\begin{figure}
  ...
  \begin{tikzpicture}[
      ...
      tapasco/.style={line width=1.5pt, draw=esa4, rounded corners=3ex, top color=esa0, bottom color=white},
      tapasco node/.style={shape=circle, text width=3mm, draw=esa5, line width=1pt, anchor=center,
    ...
    \def\td{0.275*\tapascoh}
    \coordinate (c) at (0,0);
    \coordinate (l) at (-\tapascow,0);
    \coordinate (r) at (\tapascow,0);
    \coordinate (s) at (-2*\tapascow,0);
    \coordinate (t) at (0,\tapascoh);
    \coordinate (m) at (0,0);
    \coordinate (b) at (0,-\tapascoh);

    \node [header] at (t) {Threadpool};
    %\node [header, fill=white] at (t) {Hardware Threadpool};
    \path [tapasco] (t -| l) rectangle (b -| r);
    \node [font=\Large\bfseries, anchor=center] at (c) {\vdots};
    \begin{scope}[every node/.style={tapasco node}]
      \node (in) at (m -| l) {};
      \node (out) at (m -| r) {};
      \node (ln3) at (0, 3*\td) {};
      \node (ln2) at (0, 2*\td) {};
...
\medskip
\tapasco{} adopts this notion for FPGA designs and composes \emph{hardware threadpools} (see \figref{fig:hardware-threadpool}).
...
\gloss{Compositions} are the main input to \tapasco{} and are turned into either \emph{bitstream configurations} or \emph{simulation environments}, each mode will be described in later sections.
...
\begin{figure}
  ...
  \begin{center}\textbf{FPGA Design}\\\includegraphics[]{tikz/tapasco-structure}\end{center}%
  \end{minipage}%
  \begin{minipage}{.5\textwidth}%
  \begin{center}\textbf{API Hierarchy}\\\includegraphics{tikz/tapasco-stack}\end{center}%
  \end{minipage}
...
To achieve this, the \tapasco{} APIs continue the two-tiered abstraction and provides separate APIs for \gloss{Architecture} and \gloss{Platform}, called \gloss{TPC API} and \gloss{Platform API} respectively.
...
The overall flow of \tapasco{} is depicted in \figref{fig:flow-overview} and can be divided into three distinct stages, each relying on a separate chain of tools:
...
\begin{figure}
  \centering\includegraphics[width=\textwidth]{tikz/tapasco-flow}
