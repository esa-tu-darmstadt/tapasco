...
`timescale 1 ns / 1 ps
	...
	module dual_dma_v1_0_M64_AXI #
	(
		// Users to add parameters here

		parameter integer C_M_AXI_CMD_64_WIDTH	= 32,
		parameter integer C_M_AXI_STS_64_WIDTH	= 32,
	...
	(
		...
		input wire cmd_read_m64_m_axis_tvalid,
		output reg cmd_read_m64_m_axis_tready,
		input wire [C_M_AXI_CMD_64_WIDTH - 1 : 0] cmd_read_m64_m_axis_tdata,

		input wire cmd_write_m64_m_axis_tvalid,
		output reg cmd_write_m64_m_axis_tready,
		input wire [C_M_AXI_CMD_64_WIDTH - 1 : 0] cmd_write_m64_m_axis_tdata,

		// signal from sts fifos for slave register
		output reg sts_read_m64_s_axis_tvalid,
		input wire sts_read_m64_s_axis_tready,
		output reg [C_M_AXI_STS_64_WIDTH - 1 : 0] sts_read_m64_s_axis_tdata,

		output reg sts_write_m64_s_axis_tvalid,
		input wire sts_write_m64_s_axis_tready,
		output reg [C_M_AXI_STS_64_WIDTH - 1 : 0] sts_write_m64_s_axis_tdata,

		output reg [C_M_AXI_DATA_WIDTH/8-1 : 0] m64_to_m32_dwc_s_axis_tstrb,
	...
	begin
	    M_AXI_AWVALID = 1'b0;
	    cmd_read_m64_m_axis_tready = 1'b0;
	    sts_read_m64_s_axis_tvalid = 1'b0;
		...
		IDLE:
		    begin
			if(cmd_read_m64_m_axis_tvalid == 1'b1)
			    begin				
	    			cmd_read_m64_m_axis_tready = 1'b1;			
		...
		DONE:
		    begin
			if(write_outstanding_requests == 0 && sts_read_m64_s_axis_tready == 1'b1)
			    begin
				sts_read_m64_s_axis_tvalid = 1'b1;
				next_state_write = IDLE;
			    end
			else
			    begin
	  ...
	  begin
	    if (M_AXI_ARESETN == 0)
	      begin
	        ...
	        M_AXI_AWLEN	<= 0;
		write_length <= 0;
		sts_read_m64_s_axis_tdata 	<= 0;
	      end
	    else
		if (load_write_registers == 1'b1)
	      	    begin
	        	M_AXI_AWADDR <= cmd_read_m64_m_axis_tdata[127:64];
	        	write_length <= cmd_read_m64_m_axis_tdata[63:32];
	        	sts_read_m64_s_axis_tdata <= cmd_read_m64_m_axis_tdata[31:0];
			if(cmd_read_m64_m_axis_tdata[63:32] >= BYTES_PER_BURST)
	        		M_AXI_AWLEN <= C_M_AXI_BURST_LEN - 1;
			else
			   begin
			      if(cmd_read_m64_m_axis_tdata[clogb2(BYTES_PER_BEAT)+30:32] > 0) 
	        		M_AXI_AWLEN <= cmd_read_m64_m_axis_tdata[63:32] / BYTES_PER_BEAT;
			      else
	        		M_AXI_AWLEN <= cmd_read_m64_m_axis_tdata[63:32] / BYTES_PER_BEAT - 1;
			   end
	    	    end
		else if (incr_write_registers == 1'b1)
	      	    begin
	...
	begin
	    M_AXI_ARVALID = 1'b0;
	    cmd_write_m64_m_axis_tready = 1'b0;
	    sts_write_m64_s_axis_tvalid = 1'b0;
		...
		IDLE:
		    begin
			if(cmd_write_m64_m_axis_tvalid == 1'b1)
			    begin			
	    			cmd_write_m64_m_axis_tready = 1'b1;	
		...
		DONE:
		    begin
			if(read_outstanding_requests == 0 && sts_write_m64_s_axis_tready == 1'b1)
			    begin
				sts_write_m64_s_axis_tvalid = 1'b1;
				next_state_read = IDLE;
			    end
			else
			    begin
	  ...
	  begin
	    if (M_AXI_ARESETN == 0)
	      begin
	        ...
	        M_AXI_ARLEN	<= 0;
		read_length 	<= 0;
		sts_write_m64_s_axis_tdata 	<= 0;
	      end
	    else
		if (load_read_registers == 1'b1)
	      	    begin
	        	M_AXI_ARADDR <= cmd_write_m64_m_axis_tdata[127:64];
	        	read_length <= cmd_write_m64_m_axis_tdata[63:32];
	        	sts_write_m64_s_axis_tdata <= cmd_write_m64_m_axis_tdata[31:0];
			if(cmd_write_m64_m_axis_tdata[63:32] >= BYTES_PER_BURST)
	        		M_AXI_ARLEN <= C_M_AXI_BURST_LEN - 1;
			else
			   begin
			      if(cmd_write_m64_m_axis_tdata[clogb2(BYTES_PER_BEAT)+30:32] > 0) 
	        		M_AXI_ARLEN <= cmd_write_m64_m_axis_tdata[63:32] / BYTES_PER_BEAT;
			      else
	        		M_AXI_ARLEN <= cmd_write_m64_m_axis_tdata[63:32] / BYTES_PER_BEAT - 1;
			   end
	    	    end
		else if (incr_read_registers == 1'b1)
	      	    begin
	...
	// count valid transfers reads
	  ...
	  begin
		...
		if(load_read_registers == 1'b1)
		   begin
		      read_counter <= cmd_write_m64_m_axis_tdata[63:32];
		   end
	...
	// calculate strobe of reads
	  always @(*)
	  begin
	    m64_to_m32_dwc_s_axis_tstrb <= {BYTES_PER_BEAT{1'b1}};
	    for(i = 0; i < BYTES_PER_BEAT; i = i + 1)
		begin
		   if(i >= read_counter)	   
		   	m64_to_m32_dwc_s_axis_tstrb[i] <= 1'b0;
		end
	  end
