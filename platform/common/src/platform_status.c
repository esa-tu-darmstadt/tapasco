...
static platform_res_t read_platform_status(platform_status_t **status)
{
	uint32_t d = 0;
...
platform_res_t platform_status_init(platform_status_t **status)
{
	*status = (platform_status_t *)malloc(sizeof(**status));
	...
	platform_res_t ok = read_platform_status(status);
	...
	ok = platform_addr_map_init(&(*status)->addrmap);
	if (ok != PLATFORM_SUCCESS) {
		ERR("addr map init failed: %s (%d)", platform_strerror(ok), ok);
		free(*status);
		*status = NULL;
	}
	return ok;
...
void platform_status_deinit(platform_status_t *status)
{
	if (status) {
		...
		platform_addr_map_deinit(status->addrmap);
		free(status);
	}
}
...
platform_ctl_addr_t platform_status_get_slot_base(
		const platform_status_t *status,
		platform_slot_id_t const slot_id)
{
	return platform_addr_map_get_slot_base(status->addrmap, slot_id);
}
...
{
	return platform_addr_map_get_special_base(status->addrmap, ent_id);
}
